...
class mPatchGUI extends anyC implements iGUIHTML2 {
	...
	public function processUpdate($fileName){
		...
		$updateDir = Util::getRootPath()."phynxUpdate_".date("Ymd")."/";
		...
		$this->copyDir($backupDir."specifics/", $updateDir."specifics/");
	...
	private function copyDir($dirPath, $to, $move = false){
		$dir = new DirectoryIterator($dirPath);
		foreach($dir as $file) {
			if($file->isDot())
				continue;
			...
			if($file->isDir()) {
				...
				$this->copyDir($file->getPathname(), $to.$file->getFilename()."/", $move);
